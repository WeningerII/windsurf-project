...
import { SIZE_MODS, CMB_SIZE_MODS, baseSave, classBAB } from '../shared/d20-helpers';
...
const SKILL_ABILITIES: Record<string, string> = {
  ...
  'escape-artist': 'dex', fly: 'dex', 'handle-animal': 'cha',
  heal: 'wis', intimidate: 'cha', knowledge: 'int', linguistics: 'int',
  perception: 'wis', ride: 'dex', 'sense-motive': 'wis',
  'sleight-of-hand': 'dex', spellcraft: 'int', stealth: 'dex',
  survival: 'wis', swim: 'str', 'use-magic': 'cha',
};
...
export class Pf1eEngine implements SystemEngine<Pf1eDataModel> {
  ...
  prepareData(document: CharacterDocument<Pf1eDataModel>): CharacterDocument<Pf1eDataModel> {
    ...
    d.saves.fortitude = { base: fortBase, ability: conMod, misc: d.saves.fortitude.misc, total: fortBase + conMod + d.saves.fortitude.misc };
    d.saves.reflex = { base: refBase, ability: dexMod, misc: d.saves.reflex.misc, total: refBase + dexMod + d.saves.reflex.misc };
    d.saves.will = { base: willBase, ability: wisMod, misc: d.saves.will.misc, total: willBase + wisMod + d.saves.will.misc };
    ...
    // --- AC ---
    d.armorClass.total = 10 + dexMod + sizeMod;
    d.armorClass.touch = 10 + dexMod + sizeMod;
    d.armorClass.flatFooted = 10 + sizeMod;
  ...
  async rollCheck(document: CharacterDocument<Pf1eDataModel>, checkId: string): Promise<RollResult> {
    const d = document.system;
    let modifier = 0;
    let flavor = '';
  ...
  applyDamage(document: CharacterDocument<Pf1eDataModel>, amount: number, _type: string): CharacterDocument<Pf1eDataModel> {
