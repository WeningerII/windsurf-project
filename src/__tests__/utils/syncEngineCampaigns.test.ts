...
function createSupabaseMock(
  ...
  return {
    ...
    spies: { from, select, order, upsert, eq, deleteFn, channel, on, removeChannel, authGetUser },
  };
}
...
describe('syncEngine — campaigns', () => {
  ...
  it('fetchRemoteCampaigns returns [] when Supabase is not configured', async () => {
    mockedGetSupabaseClient.mockReturnValue(null);
    await expect(fetchRemoteCampaigns()).resolves.toEqual([]);
  });

  it('fetchRemoteCampaigns maps remote rows to local Campaign shape', async () => {
    ...
    const result = await fetchRemoteCampaigns();
  ...
  it('pushCampaigns batches an array and short-circuits on empty input', async () => {
    const mock = createSupabaseMock();
    mockedGetSupabaseClient.mockReturnValue(mock.client);
  ...
  it('deleteRemoteCampaign targets campaigns table by id', async () => {
    const mock = createSupabaseMock();
    mockedGetSupabaseClient.mockReturnValue(mock.client);
    ...
    expect(mock.spies.eq).toHaveBeenCalledWith('id', 'c1');
  });
