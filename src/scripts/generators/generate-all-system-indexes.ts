...
const dnd5eConfig = {
  ...
  levels: ['cantrips', 'level-1', 'level-2', 'level-3', 'level-4', 'level-5', 'level-6', 'level-7', 'level-8', 'level-9'],
...
const pf2eConfig = {
  ...
  levels: ['cantrips', 'level-1', 'level-2', 'level-3', 'level-4', 'level-5', 'level-6', 'level-7', 'level-8', 'level-9', 'level-10'],
...
const pf1eConfig = {
  ...
  levels: ['level-0', 'level-1', 'level-2', 'level-3', 'level-4', 'level-5', 'level-6', 'level-7', 'level-8', 'level-9'],
  schools: ['abjuration', 'conjuration', 'divination', 'enchantment', 'evocation', 'illusion', 'necromancy', 'transmutation'],
  type: 'school'
};
...
function createLevelIndexForSchools(level: string, schools: string[], systemPath: string, systemName: string): void {
...
function createLevelIndexForTraditions(level: string, traditions: string[], systemPath: string, systemName: string): void {
  const levelDir = path.join(systemPath, level);
  const indexPath = path.join(levelDir, 'index.ts');
...
console.log('\n📊 Summary:');
console.log(`   Pathfinder 2e: ${pf2eConfig.levels.length} levels × ${pf2eConfig.traditions.length} traditions = ${pf2eConfig.levels.length * pf2eConfig.traditions.length} folders`);
console.log(`   Pathfinder 1e: ${pf1eConfig.levels.length} levels × ${pf1eConfig.schools.length} schools = ${pf1eConfig.levels.length * pf1eConfig.schools.length} folders`);
