 ...
 * which entries claim which source. Every entry it emits is open content by
 * construction: the loaders filter on `isOpenContentCompliant`, whose only
 * admission list is `allowedSources`. (Until 2026-07-30 there was a second
 * channel for self-authored entries, emitted here with status 'original'; that
 * content and that channel are both gone.) This generator never invents entries.
 */
...
// Entries: ${count}, all open content — the loaders admit nothing else.
