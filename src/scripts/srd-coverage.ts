#!/usr/bin/env tsx
/**
 * DENOMINATOR A. Fetch INDEPENDENT open-content SRD entry lists (hosted on
 * GitHub raw — verified fetchable from this environment via Node, unlike the
 * truncating WebFetch tool) and diff them against the product's loaders by
 * normalized name. The denominator comes from OUTSIDE the repo, so it measures
 * real coverage: which SRD entries the product is MISSING. Run with network;
 * writes docs/generated/srd-coverage.md plus the machine-readable
 * docs/generated/srd-coverage.json that offline consumers read (both committed
 * static). Re-run to refresh.
 *
 * This superseded the loader-derived docs/srd-manifest/ catalogs as Denominator
 * A (decision 2026-07-21, executed 2026-07-27 — docs/GAPS.md §6): because those
 * manifests are GENERATED FROM the loaders, joining their ids against loaded ids
 * put the same population on both sides of the ratio and could only ever read
 * 100%. The manifests are now provenance-only.
 *
...
async function main(): Promise<void> {
  ...
  lines.push('');
  lines.push(
    `_Generated by \`npm run srd:coverage\` (requires network). Independent open-content SRD entry lists fetched from GitHub-hosted sources and diffed against the product loaders by normalized name. The denominator comes from OUTSIDE the repo, so this is REAL coverage — which SRD entries the product is missing. **This file is Denominator A** — the sole content denominator. \`docs/generated/roadmap-metrics.md\` republishes a per-system rollup of it, read offline from the \`srd-coverage.json\` sidecar written by this same run. The loader-derived \`docs/srd-manifest/\` catalogs it replaced are provenance-only — see \`docs/GAPS.md\` §6._`
  );
  lines.push('');
  ...
  const out = path.resolve(repoRoot, 'docs/generated/srd-coverage.md');
  await fs.writeFile(out, `${lines.join('\n')}\n`, 'utf8');
  console.log(`\nWrote ${path.relative(repoRoot, out)}`);

  // Machine-readable sidecar. This exists so OFFLINE consumers — chiefly
  // `src/scripts/generate-roadmap-metrics.ts`, which runs on every
  // `check:generated-docs` and must never need the network — can publish
  // Denominator A from THIS measurement instead of re-deriving one. The
  // committed JSON is the offline record of the last networked run; it moves
  // only when someone re-runs `npm run srd:coverage`.
  const jsonOut = path.resolve(repoRoot, 'docs/generated/srd-coverage.json');
  const payload = {
    generatedAt: new Date().toISOString(),
    note: 'Independent SRD reverse diff (Denominator A). Written by `npm run srd:coverage` (networked) alongside srd-coverage.md; read offline by generate-roadmap-metrics.ts. Not hand-editable.',
    rows: rows.map((r) => ({
      systemId: r.systemId,
      systemLabel: r.systemLabel,
      category: r.category,
      srdSource: r.srdSource,
      covered: r.covered,
      srdTotal: r.srdCount,
      pct: r.pct,
      missingCount: r.missing.length,
      loaderCount: r.loaderCount,
      overInclusionCount: r.extra.length,
    })),
    absent: ABSENT.map((a) => ({ systemLabel: a.systemLabel, category: a.category })),
  };
