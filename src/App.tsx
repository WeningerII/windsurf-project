...
import { DragRoot } from './components/drag/DragRoot';
...
function AppContent() {
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* Phase-4 drag boundary: with the scene-drag flag OFF (the production
          default) this renders children verbatim and imports nothing, so the
          eager index chunk and the shell DOM are unchanged. With the flag on it
          lazy-loads a DragProvider wrapping the whole shell so the Dock (drag
          source) and the SurfaceStage-hosted Scene grid (drop target) share one
          drag context. */}
      <DragRoot>
        <a
          href="#main-content"
