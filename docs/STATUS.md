...
- The prompt box is LLM-authored across all 7 systems via a server-gated build gateway (a Netlify function holding the key, model id from operator env). The browser hands the model a compact, machine-readable options manifest (the real catalog names + the system's structural rules) and it returns a full BuildSpec — class, ancestry/species, ability allocation, spells/cards, name. Each per-system creator re-validates the model's selections against the real catalogs and uses them where they resolve, falling back per field to its deterministic logic; the engine then derives and the validator judges. A validate-and-repair loop then closes the fidelity gap: any validator errors plus name picks that didn't resolve (a silent fallback) are fed back to the model with its previous selections so it corrects its own attempt, for a bounded number of rounds, keeping the best result. So the model genuinely designs the character (a "Batman" becomes a stealthy Rogue, not a keyword-nearest Bard) — and is told to fix a bad pick rather than have it quietly replaced — but it can never produce an illegal sheet, and with no key the prompt box degrades to the deterministic builder. This is the BEFORE-mechanics half of "AI proposes, the rules decide" (the combat narrator is the AFTER half). A local dev runner (`scripts/ai-create.ts`) exercises the live path with a key.
