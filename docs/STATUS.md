...
- That creation flow has an optional server-gated LLM "drafting" path (a Netlify function holding the key) that turns a description into structured hints which only steer catalog selection; the deterministic core still derives and the validator still judges, so the model can never produce an illegal sheet. With no key the prompt box runs fully offline on its keyword parser. This is the BEFORE-mechanics half of "AI proposes, the rules decide" (the combat narrator is the AFTER half).
