...
Content (Denominator A) is enumerated as cited per-entry manifests in `docs/srd-manifest/` (generated by `src/scripts/generate-srd-manifests.ts`) — 4,216 open-content entries across the seven systems, every one encoded, loader-backed, source-tagged, and open-content-policy-clean — giving 100% **catalog parity** (the metric's content%). Full-SRD *coverage* (detecting entries the published SRD contains but the loaders omit) is a stricter, separate dimension. It is no longer blocked: the container's Node `fetch()` retrieves the open-content SRD datasets from GitHub raw in full (the `WebFetch` *tool* truncates; Node does not), and `npm run srd:coverage` builds a genuine independent-coverage report at `docs/generated/srd-coverage.md` by diffing those lists against the loaders, each scoped to the policy's `allowedSources`. **Six of the seven systems are now measured:** M&M 3e (powers 40/40, advantages 73/73) and Daggerheart (domain cards 189/189, domains 9/9) are at *genuine* 100% on the wired categories, while the Pathfinder and 5e *spell* catalogs show real gaps (PF2e Core spells 24%, PF1e Core spells 21%, 5e‑2014 spells 67%) — plus a 5e provenance over-inclusion (see `docs/GAPS.md`). Only D&D 3.5e is unwired (its `D35E` source mixes core with Psionics/Epic and isn't cleanly scopable yet). Until the remaining categories are wired and the genuine numbers are folded into the headline metric, the catalog-parity 100% certifies provenance + parity, **not** completeness against the full published SRD — the honest coverage lives in `srd-coverage.md`.
