...
**Current state:** Engine math (Denominator B) is verified at 100% across all seven systems — every derived quantity in the compute registers is test-pinned. The registers were rebalanced to **even depth** so no system is neglected: each now holds 23–28 verified quantities spanning layers L1–L10 (previously the registers were lopsided, with D&D 5e (2014) at 28 while Daggerheart held only 13). The rebalancing added genuine, SRD-cited quantities rather than padding — e.g. shared d20 carrying-capacity / encumbrance / bonus-spell math cited by both 3.5e and PF1e; the PF2e dying/wounded/recovery track and encounter XP budget; M&M check resolution, Affliction DC, equipment and hero points; Daggerheart's damage-reduction pipeline, death moves, and character-creation baselines. Every added value was verified against the cited open-content SRD (for the less-certain PF2e encounter and Daggerheart numbers, against the live open-content sources via Node `fetch`).

Content (Denominator A) is enumerated as cited per-entry manifests in `docs/srd-manifest/` (generated by `src/scripts/generate-srd-manifests.ts`) — 4,216 open-content entries across the seven systems, every one encoded, loader-backed, source-tagged, and open-content-policy-clean — giving 100% **catalog parity** (the metric's content%). Full-SRD *coverage* (detecting entries the published SRD contains but the loaders omit) is a stricter, separate dimension. It is no longer blocked: the container's Node `fetch()` retrieves the open-content SRD datasets from GitHub raw in full (the `WebFetch` *tool* truncates; Node does not), and `npm run srd:coverage` builds a genuine independent-coverage report at `docs/generated/srd-coverage.md` by diffing those lists against the loaders. D&D 5e (both editions) is measured (revealing real spell/monster/equipment gaps and a provenance over-inclusion — see `docs/GAPS.md`); the other five systems' independent sources are listed in `docs/srd-sources.md` and their wiring is in progress. Until all seven are wired and folded in, the headline 100% content figure certifies catalog parity + provenance, **not** completeness against the full published SRD.
