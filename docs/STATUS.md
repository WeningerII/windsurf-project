...
- **Manual boundaries** — `docs/srd-manifest/_exclusions.ts`: the enumerated manual, reference-only, and narrative items excluded from both denominators, so that fake automation cannot be counted as coverage. **Corrected 2026-07-26:** the registry currently holds 10 entries and does not yet cover every residual gap — 4 of 7 systems carry measured content shortfalls that are neither automated nor enumerated here, and one carries no entries at all despite naming a manual surface in its support row. The exclusion mechanism does what it claims; the claim that the metric is *therefore* never gameable outran it. Live shortfalls are in `docs/generated/srd-coverage.md` and tracked in `docs/GAPS.md` §1.
