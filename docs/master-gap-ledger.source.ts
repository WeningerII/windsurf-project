...
export const MASTER_GAP_LEDGER: readonly GapLedgerItem[] = [
  ...
  // ── Phase 1 — Denominator A (content) ────────────────────────────────────
  {
    ...
    phase: 1,
    track: 'A-content',
    title: 'Encode Mutants & Masterminds equipment',
    detail:
      'M&M equipment (weapons/armor/grenades/vehicles/HQ) is not wired in srd-coverage; encode from the frnprt/mm3e-character-creator EQUIPMENT source. The single largest content gap.',
    status: 'missing',
    tags: ['engineering'],
    system: 'mam3e',
    evidence: 'docs/srd-sources.md (M&M); docs/generated/srd-coverage.md',
  },
  {
  ...
  },
  {
    ...
    evidence: 'docs/generated/srd-coverage.md (monster rows + Pending note)',
  },
  {
  ...
  // ── Phase 3 — Denominator B (engine-math wiring) ─────────────────────────
  {
    ...
    detail:
      'check:compute-register --mutate proves 44 of the 186 verified entries today: proficiency/AC/saves, 5e spell save DC + attack, the full L3 damage-assembly set (5e riders rage/Divine Smite/GWM/Sharpshooter/Extra Attack, 3.5e/PF1e iteratives + crit confirmation, M&M attack/resistance/affliction/critical/degrees DCs, Daggerheart weapon/critical/Spellcast damage dice), and the d20 offense leaves (3.5e/PF1e BAB tracks, grapple, CMB, CMD, maneuver resolution; PF2e MAP, striking runes, attack modifier). Author a real formula mutation anchor per remaining verified entry so "verified" graduates from test-linked (Tier A) to mutation-proven across all systems.',
    status: 'in-progress',
  ...
  },
  {
    ...
    detail:
      'Sneak Attack / Rage / Divine Smite / GWM / Sharpshooter / Extra Attack counts, 3.5e/PF1e iteratives + crit confirmation, Daggerheart crit damage + Spellcast dice, and M&M Damage/Affliction DCs are wired through the seeded dice substrate + resolver and mutation-proven. REMAINING: two-weapon and versatile damage, which need the weapon-data-driven 5e damage path first — buildCharacterCombatant still rolls a baseline weaponDie default, not the equipped weapon dice.',
    status: 'pending',
    tags: ['engineering'],
    evidence:
      'docs/compute-register/* (L3 entries mutation-proven); src/rules/resolver/attackResolution.ts (crit confirmation); src/__tests__/rules/attackResolution.test.ts',
  },
  {
