...
export const MASTER_GAP_LEDGER: readonly GapLedgerItem[] = [
  ...
  // ── Phase 4 — cross-system parity & UX ───────────────────────────────────
  {
    ...
    detail:
      "RE-SCOPED 2026-08-02 against code: THREE of the five items are already done and this entry had not been re-read since they landed. DONE: (1) 5e ledger spell-DC/attack rows exist in src/systems/dnd5e/shared/contributionLedger.ts and reach BOTH editions through useDnd5eContributionLedger, so the '5e-2024' framing was moot — the builder is shared; (2) d20-legacy ledger breadth ships BAB, saving-throw and 3.5e skill-synergy rows, with the PF1e-has-no-synergy-subsystem nuance stated on the rows themselves; (3) all seven systems expose loadCreationPlan and M&M has MamConditionsTab.tsx, so 'the one system lacking a creator' is no longer true. REMAINING, and both are the same shape — declared but never wired, the defect class that also hid legalActions.ts and pf2e/derivedMath.ts: (4) PF2e MULTICLASS AGGREGATION — src/systems/pf2e/data-model.ts declares the dedication-grant types (which proficiency map a grant trains, and the grant itself) and NOTHING consumes them; grep for 'multiclass' across src/systems/pf2e/ returns only the data model, so a dedication feat's proficiency grants never reach derived proficiencies; (5) FEATURE-OPTION EFFECT SURFACING — featureOptions are handled entirely in the 5e sheet layer (dnd5eSheetShared, FeatureOptionBrowser, Dnd5eFeatureOptionsSection, validation) with ZERO consumers under src/rules/, so a chosen feature option's mechanical effect never reaches the resolver and cannot appear in a contribution ledger.",
    status: 'pending',
    tags: ['engineering'],
