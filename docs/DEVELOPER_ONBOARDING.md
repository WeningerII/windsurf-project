...
```bash
npm run check:node  # Enforce local runtime parity for build/coverage/e2e
...
├── hooks/               # React hooks (useDocuments primary path)
├── systems/             # System definitions, engines, and sheet bindings
├── registry/            # SystemRegistry + shared system interfaces
...
| File | What It Does |
|------|-------------|
| `src/types/core/document.ts` | V2 `CharacterDocument<T>` + `SystemDataModel` |
| `src/types/game-systems.ts` | `GameSystemId` union type + re-exports |
| `src/utils/documentStorage.ts` | V2 localStorage persistence (`rpg-documents-v2`) |
| `src/utils/storage.ts` | Legacy localStorage layer kept for migration |
| `src/hooks/useDocuments.ts` | Primary React hook for document CRUD |
| `src/App.tsx` | Main application component |
| `src/components/SystemSheetRenderer.tsx` | Dispatches to system-specific sheet component |
| `src/components/CharacterSheetTabs.tsx` | Tabbed character sheet view (shared d20 path) |
| `src/utils/dataLoader.ts` | Dynamic data loading by system |

### Architecture Layers

```
Data (src/data/)  →  Loaders (src/utils/)  →  Hooks (src/hooks/)  →  Components (src/components/)
     ↑                                                                        ↑
  Types (src/types/) + Registry/Systems                                UI (TailwindCSS + shadcn/ui)
```

- **Data layer**: Pure static data arrays. No logic.
- **Loaders**: Dynamic imports by game system. Handle aggregation.
- **Hooks**: Business logic and state management. `useDocuments` is the primary hook.
...
| `npm run check:node` | Verify Node runtime compatibility (`^20.19 || ^22.12 || >=24`) |
...
| `refactoring/LEGACY_DEPRECATION_PLAN.md` | Legacy hook/component/storage removal phases |
| `character-schema.md` | Character data model reference |
| `data-export-format.md` | localStorage + export/import format |
| `future-backend-api.md` | API contract for future backend |
