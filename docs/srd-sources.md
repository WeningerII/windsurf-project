...
- **Machine-readable (verified):** `Rughalt/D35E` (Foundry, OGL), NeDB JSON-lines under `packs/` — `https://raw.githubusercontent.com/Rughalt/D35E/master/packs/spells.db` (**673**, incl. psionics+epic), `feats.db` (388), `bestiary.db` (645), `classes.db` (33), + magic-items/weapons/armor. Also `olimot/srd-v3.5-md` (clean Markdown, OGL) — **wired as the core-only spell + monster denominator in `npm run srd:coverage`** (the `## ` chapter headers). The monster `## ` headers include taxonomic CATEGORY containers (Angel/Dragon/Elemental/…) that nest separately-named members plus age/size variant rows; `collapse35eMonsterHeadings` (`src/scripts/srdCoverageShape.ts`) drops the category containers and folds age/size variants to their archetype so the denominator counts individual stat blocks (confirmed individuals like Salamander/Hydra/Lich/Ghost are preserved as genuine misses, never masked as containers). **Scope caveat:** D35E is SRD 3.5 **plus** Psionics + Epic, so it is intentionally avoided in favor of the core-only olimot chapters.
