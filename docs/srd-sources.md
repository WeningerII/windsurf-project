...
- **Machine-readable (verified):** `Rughalt/D35E` (Foundry, OGL), NeDB JSON-lines under `packs/` — `https://raw.githubusercontent.com/Rughalt/D35E/master/packs/spells.db` (**673**, incl. psionics+epic), `feats.db` (388), `bestiary.db` (645), `classes.db` (33), + magic-items/weapons/armor. Also `olimot/srd-v3.5-md` (clean Markdown, OGL) — **wired as the core-only spell + monster denominator in `npm run srd:coverage`** (the `## ` chapter headers). The monster `## ` headers include taxonomic CATEGORY containers (Angel/Dragon/Elemental/Ooze/Snake/Sprite/Swarm/Planetouched/…) that nest separately-named members plus age/size variant rows, and TEMPLATE headers that carry no stat table at all (Celestial/Fiendish Creature, Half-Celestial, Half-Fiend); `collapse35eMonsterHeadings` (`src/scripts/srdCoverageShape.ts`) drops both kinds and folds age/size variants to their archetype so the denominator counts individual stat blocks. Container membership is decided STRUCTURALLY against the upstream chapters — a heading is a container only when its `## ` section is prose-only and every stat block sits under a separately-named `### ` child. Confirmed individuals are preserved as genuine misses, never masked as containers: Salamander/Hydra/Lich/Ghost, and also Vampire/Skeleton/Zombie/Half-Dragon (their sections do print a stat block) and Fungus/Arrowhawk/Animated Object (whose `## ` section owns one combined multi-column table). **Scope caveat:** D35E is SRD 3.5 **plus** Psionics + Epic, so it is intentionally avoided in favor of the core-only olimot chapters. The olimot chapters are also the ENCODER source for 3.5e content, not only the denominator: `scripts/encode-35e-spells.mjs`, `scripts/encode-35e-monsters.mjs`, and `scripts/encode-35e-feats.mjs` transcribe from them. The feats encoder reads the `### Name <small>[Type]</small>` blocks under `## Feat Descriptions` — the same discrimination `fetchSrd35FeatNames` applies — so the encoded catalog and the measured denominator can never drift apart; hand-written feats win on name/id and the encoder only adds what the SRD has and the repo lacks.
