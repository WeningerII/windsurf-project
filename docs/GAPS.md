...
- **Several "verified" quantities are tested helpers, not engine-wired.** Spell
  ...
  and max-rank enforcement are now wired** — synergy applies in both the skills
  tab and `rollCheck`; the skills tab shows each skill's RAW rank cap and flags
  over-cap values. Conditional and Knowledge-subtype synergies stay manual.)
