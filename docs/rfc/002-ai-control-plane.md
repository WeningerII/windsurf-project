...
Provider selection, mock provider, request rate limiting, request authentication, and structured observability have since landed (all pure, injectable, and SDK-free):
...
- **Structured logs** (`src/ai/gatewayLog.ts`): trace id, task, provider/adapter id, latency, failure kind, and repair count, emitted once at each terminal branch through an injectable sink (tests assert without stdout noise; the default sink uses `console.warn`/`console.error`).

Still target design, not yet built: AI character-concept draft and draft repair (await deterministic guided creation), rule/provenance explanation, tactical intent hints, and the deeper cost-control layer (cost buckets, per-session ceilings). Delivering these against a hosted provider still requires real keys and remains out of scope for the local/CI slice above; only per-request timeout, key-less/error degradation, request rate limiting, and the observability fields are implemented so far. The task allowlist grows one entry at a time as each remaining surface lands with its tests.
