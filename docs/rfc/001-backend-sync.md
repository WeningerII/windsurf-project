...
- Use Supabase Edge Functions, not the client bundle. The same TypeScript validation modules already used by the React app (`src/systems/d20-legacy/d20LegacyTemplate.ts`, `src/systems/pf2e/data-model.ts`, etc.) move to a new shared validation directory under `src/` consumable by both runtimes. The exact path is not pre-allocated here; it is created at the time the work lands so this RFC does not assert a path that does not yet exist.
