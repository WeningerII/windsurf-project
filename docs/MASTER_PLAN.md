...
> a data model that keys choices by NAME, not id. **Six of seven systems are now
> creatable** (5e 2014, 5e 2024, PF2e, 3.5e, PF1e, Daggerheart). M&M 3e is the
> remaining system, and it is a different shape — point-buy power-point allocation
> rather than catalog picks — so it is evaluated separately (see the 4C row) rather
> than forced into a pick-one flow. The 5e-named rows in the legacy table below are
...
| 4C. Remaining Systems' Orchestrators | Apply the same registry pattern to the other systems | each system's loaders/templates/validators | **PF2e + d20-legacy + Daggerheart DONE (2026-06-23):** `pf2eCreation.ts` (ancestry → background → class), `d20LegacyCreation.ts` (one race → class flow for BOTH 3.5e and PF1e), and `daggerheartCreation.ts` (class → ancestry → community, name-keyed model) register orchestrators the SAME shared creator renders unchanged — proving the contract across different data models, step orders, and loaders. Six of seven systems are creatable; 5e 2014 falls out of the edition-agnostic 5e orchestrator (no separate fork). Remaining: M&M 3e, whose creation is point-buy power-point allocation (not catalog picks) — to be assessed as either a thin archetype-pick orchestrator or an enumerated "point-buy on the sheet" boundary, never a faked pick-one flow | Per-system creation fixtures; the shared creator renders each system without modification; no regression to existing flows |
