...
> validators and template applicators — not further resolver work. **That work
> has now begun shipping:** the system-agnostic draft shell (Phase 4A — pure
> `CreationDraft` reducer + resumable local persistence, `src/creation/`) and a
> user-visible D&D 5e guided-creation wizard covering class → species →
> background → review (Phase 4B core, `src/components/CharacterCreationWizard.tsx`)
> are live. The wizard drives the SAME template applicators and registry
> validator the sheet uses (no parallel rules), displays validation issues live
> and gates finalisation on errors, resumes an in-progress draft after a refresh,
> and emits a normal `CharacterDocument`. The flow is edition-agnostic, so it also
> satisfies the Phase 4C 2014 reuse for this path. What remains in THIS track is
> the wizard's ability-planning / spell / equipment steps "where current data
> supports it," and per-system creation orchestrators for the non-5e systems —
> not further resolver work.
...
| 4A. Wizard Architecture | New UX and draft state | character creation entry points, local storage layer, template handlers | **DONE (2026-06-22).** Deterministic guided-creation shell (`src/creation/creationDraft.ts` + `creationDraftStorage.ts`): pure system-agnostic draft reducer with step state, selection bookkeeping, validation-issue absorption, resumable local persistence, reset/cancel, and final `CharacterDocument` output | Draft resume tests, reset/cancel tests, no Supabase dependency, no new remote schema |
| 4B. D&D 5e 2024 Wizard Path | First user-visible guided creator | 5e 2024 loaders/templates, ability/spell/equipment surfaces where supported | **Core path DONE (2026-06-22):** a user-visible class → species → background → review wizard (`src/creation/dnd5eCreation.ts` orchestrator + `src/components/CharacterCreationWizard.tsx`), reachable from the landing for 5e, driving the SAME applicators/validator the sheet uses and finalising a normal `CharacterDocument`. Remaining 4B: ability planning, spell, and equipment steps "where current data supports it" (today completed on the sheet after creation) | Template application tests, validation failure display, import/export roundtrip, existing 5e engine/template regressions |
| 4C. D&D 5e 2014 Reuse | Compatibility pass, not a fork unless necessary | 5e 2014 loaders/templates and shared 5e wizard components | **Core path satisfied (2026-06-22):** the 5e orchestrator + wizard are edition-agnostic (one flow serves 2014 and 2024 via `Dnd5eLikeDataModel` + the shared applicators), so the 2014 guided path is the same component. Edition-specific later steps isolate only if a future ability/spell/equipment step diverges | 2014 fixture coverage for class/species/background/feat/feature-option paths; no regression to 2024 flow |
