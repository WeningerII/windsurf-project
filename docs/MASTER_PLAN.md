...
The `Support level` column above is a **current-state snapshot, not a permanent boundary.** Under the all-systems-equal direction, `Partial` marks where deterministic RAW auto-resolution is still being built, not a system that is intentionally lesser. Six of the seven systems are now `Full`; Daggerheart remains `Partial` because its triggered/timing/choice/narrative card effects are GM-adjudicated by design (an accepted boundary), with only its passive automation deterministic. A system reads `Full` when its only residual gaps are the enumerated accepted manual boundaries in `docs/srd-manifest/_exclusions.ts` (e.g. Vancian prepared-slot assignment and spontaneous conversion for the d20 casters), never unfinished automation.
