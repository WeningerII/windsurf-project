...
- The next major product program is the system-agnostic rules IR and deterministic effect resolver (`docs/rfc/003-rules-ir-and-effects.md`), and on top of it rule truth, provenance, and executable activities for all seven systems equally. It is not just an AI spike: it is a multi-phase implementation program across a shared effect resolver, per-system effect compilers, validation, derivation, and action execution — built once and applied to every system rather than deepened one system at a time.
...
- The provider-free AI control-plane scaffolding and an early guided-creation draft substrate were removed on 2026-05-30: both were orphaned (reachable from no UI) and the AI gateway was a non-functional stub. At that time the AI-DM track below remained future work; the AI port and its first task surfaces later shipped (2026-06-19) from the deterministic foundation, not from the removed scaffolding.
...
- `Active implementation track`: AI-DM integration follows the same control-plane contract as AI character creation. Prompted encounter specs, map/vision proposals, and narration are structured drafts or read-only artifacts until deterministic validators, typed scene actions, and replayable events accept them.
