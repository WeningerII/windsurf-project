...
- The next major product program is the system-agnostic rules IR and deterministic effect resolver (`docs/rfc/003-rules-ir-and-effects.md`), and on top of it rule truth, provenance, executable activities, and guided creation for all seven systems equally. It is not a cosmetic creation UI and it is not just an AI spike: it is a multi-phase implementation program across a shared effect resolver, per-system effect compilers, validation, derivation, action execution, draft persistence, template application, and UX — built once and applied to every system rather than deepened one system at a time.
...
| D&D 3.5e | Full | Shared legacy sheet, full core prestige catalog is selectable, canonical 610-spell loader-backed catalog with alias-safe class-stub duplicate collapse, source-backed legacy spell metadata, deterministic spells-per-day (casting-ability, cleric domain, wizard specialist, and prestige/Dragon-Disciple bonus slots), synergy/encumbrance/gear skill resolution, equipped-armor AC; Vancian prepared-slot assignment and spontaneous conversion remain accepted manual boundaries; no monster product surface |
| Pathfinder 1e | Full | Shared legacy sheet, vetted prestige support is product-reachable, raw `levelsByClass` and source-backed legacy spell metadata live in spell files, deterministic spells-per-day (casting-ability, cleric domain, wizard specialist, and prestige/Dragon-Disciple bonus slots), class-skill/encumbrance/gear skill resolution, equipped-armor AC; Vancian prepared-slot assignment and spontaneous conversion remain accepted manual boundaries; no monster product surface |
...
- `Completed foundation`: legacy d20 prestige spellcasting, Vancian preparation, deterministic domain/specialist/Dragon-Disciple bonus-slot counting, and no-monster reporting boundaries are regression-gated inside the shared `d20-legacy` path. Only prepared-slot assignment and spontaneous conversion remain manual, as accepted boundaries.
...
> character creation** (the creation rows in the table below) on top of the
> shipped validators and template applicators — not further resolver work. **That
> work has begun shipping, system-agnostically:** the creation shell (Phase 4A —
> pure `CreationDraft` reducer + resumable local persistence, `src/creation/`) and
> one **system-agnostic character creator** (`src/components/CharacterCreator.tsx`)
> are live. The creator owns no per-system rules: it resolves the current system's
> creation orchestrator from the registry (`SystemDefinition.creation`, the same
> dispatch pattern as engines and validators) and walks that system's steps,
> driving the SAME template applicators and registry validator the sheet uses (no
> parallel rules), displaying validation issues live, gating finalisation on
> errors, resuming an in-progress draft after a refresh, and emitting a normal
...
> class flow serving BOTH D&D 3.5e and PF1e; and **Daggerheart is the fourth**
> (`src/creation/daggerheartCreation.ts`), a class → ancestry → community flow over
> a data model that keys choices by NAME, not id. **Six of seven systems have a
> guided creator** (5e 2014, 5e 2024, PF2e, 3.5e, PF1e, Daggerheart). The seventh,
> **M&M 3e, is a deliberate, enumerated boundary, not a gap:** its creation is
> point-buy power-point allocation, not catalog picks, and it has no "apply a
> template" surface (archetypes are toggled reference suggestions, not
> character-defining templates) — so a pick-one orchestrator would require inventing
> new creation rules, which the no-parallel-rules constraint forbids. M&M therefore
> uses quick blank-create (point-buy on the sheet). With that decision recorded,
> **the creation track (4A/4B/4C) is complete.** The 5e-named rows in the legacy
> table below are
...
>
> **Validation parity (2026-06-23): DONE — all seven systems have a registry
> validator.** A per-capability parity audit (`docs/PARITY_AUDIT.md`) found the one
> genuine cross-system spike was the registry validator (previously 5e-only); it is
> now closed everywhere: **M&M 3e** (`src/systems/mam3e/validation.ts`, PL
> trade-off caps + power-point budget from the engine's own math), **Daggerheart**
> (`src/systems/daggerheart/validation.ts`, SRD creation trait array + Hope bounds
> + class/ancestry legality), **D&D 3.5e + Pathfinder 1e** (one shared
> `src/systems/d20-legacy/validation.ts`: level bounds, race/class id resolution,
> per-class level bounds, multiclass level-sum rule), and **PF2e**
> (`src/systems/pf2e/validation.ts`: ancestry/heritage/background/class id
> resolution + level bounds). Each reuses its system's loaders/engine math — no
> parallel rules. The remaining tracked parity item is the contribution-breakdown
> ledger for PF2e/3.5e/PF1e (M&M/Daggerheart already have one); the
> resolver-routing/conditions/M&M-creation absences are the intentional terminal
> boundaries above, not gaps.
...
| 4C. Remaining Systems' Orchestrators | Apply the same registry pattern to the other systems | each system's loaders/templates/validators | **PF2e + d20-legacy + Daggerheart DONE (2026-06-23):** `pf2eCreation.ts` (ancestry → background → class), `d20LegacyCreation.ts` (one race → class flow for BOTH 3.5e and PF1e), and `daggerheartCreation.ts` (class → ancestry → community, name-keyed model) register orchestrators the SAME shared creator renders unchanged — proving the contract across different data models, step orders, and loaders. Six of seven systems are creatable; 5e 2014 falls out of the edition-agnostic 5e orchestrator (no separate fork). **M&M 3e is a closed, enumerated boundary (2026-06-23), not remaining work:** its creation is point-buy power-point allocation with no template-apply surface (archetypes are toggled reference suggestions), so it uses quick blank-create rather than a faked pick-one flow — recording this completes the creation track | Per-system creation fixtures; the shared creator renders each system without modification; no regression to existing flows |
...
- `Shipped (2026-06-19)`: **Phase 7 (Server-Side AI Port)** and **Phase 8 (AI Encounter Spec Drafting)** are live — a provider-agnostic gateway (`src/ai/`, server adapter in `netlify/functions/`, Vercel AI SDK + Gemini), default-off behind `VITE_AI_ENABLED`, with fixture-replay CI, key-less degradation, and encounter drafting gated by the shipped encounter-spec validator. Three further task surfaces landed as the **first slices** of later modalities: recap narration (the Phase 13 narration slice; its faithfulness critic shipped 2026-06-22, below), image-based creature identification (a Phase 10 vision slice; grid/geometry automation now ships, below), and scene illustration (a Phase 9 image-generation slice — the full map-asset pipeline now ships, below). Still open on this track: the full map-asset pipeline (9), grid-geometry automation (10), and the observability/cost-control layer (14) — only a per-request timeout and key-less/error degradation exist so far.
- `Shipped (2026-06-21)`: **Phase 11 (Local Tactical AI Executor)** is live — deterministic NPC turns with no LLM in the per-move hot path. Pure utility target scoring (`src/rules/tactical/targetScoring.ts`), a per-turn decide-and-resolve executor with Multiattack/iterative attacks (`tacticalExecutor.ts`), and an initiative round driver (`roundDriver.ts`) that threads working HP and positions. Movement is obstacle-aware: a deterministic Chebyshev pathfinder (`pathfinding.ts`) routes around creatures and the grid edge, is footprint-aware for large creatures, and only ever returns a legal scene placement — so every move the autonomous round proposes survives the runtime's re-validation in `applySceneIntents` (closing a phantom-damage gap where a rejected straight-line move still let its attack land), and a boxed-in actor falls back to closest-approach. A local influence reading (engagement count) positions an actor among equally-fast cells to threaten the most enemies. Invalid simulated intents are reported, not dropped, and every decision replays byte-identically under seed.
- `Shipped (2026-06-21)`: **Phase 12 (LLM Strategist Blackboard)** is live — an optional async strategist that biases, but never overrides, the Phase 11 executor. A deterministic blackboard (`src/rules/tactical/strategy.ts`) carries per-actor target-priority hints; `scoreTargets` folds a CLAMPED bias into matching targets only AFTER the hostile/living filter, so a hint can reorder which already-legal target is chosen but can never make a dead, allied, or out-of-reach target actable. Hints are stamped with a round and ignored when stale or missing, so an absent strategist leaves the pure heuristic untouched. The model side is a fifth control-plane task (`strategy-hints`): `strategistFlow.requestStrategyHints` turns a compact combat snapshot into a validated, round-stamped board, keeping only hints that name a real actor on the advised side targeting a real combatant. It is wired into the scene's combat panel behind an "AI strategist" toggle (AI-gated) and runs BETWEEN rounds — fire-and-forget off the post-round state — so no turn ever blocks on a model call and AI-off/keyless play is byte-identical to the deterministic baseline.
- `Shipped (2026-06-22)`: **Phase 13 (AI-DM Narration & Critic)** is complete — the narration slice gains its faithfulness critic. AI narration may only restyle the deterministic recap (`summarizeSceneForLog`), so a faithful retelling introduces no facts of its own. A pure, deterministic critic (`src/ai/narrationCritic.ts`) is the validator: it flags digit-runs absent from the recap (false mechanical claims) and mid-sentence Title-Case words absent from the recap (invented characters/places), tuned for precision (sentence-initial capitals and acronyms are never flagged; recap words are always allowed). The narration flow runs it, sends ONE bounded rewrite citing the issues, re-checks, and on continued failure falls back to the deterministic recap as the prose (surfaced in the recap panel) — so an unfaithful draft is corrected or replaced by honest facts, never shown as if grounded. The critic is replayable and cost-free (no second model), never touches events (narration is read-only), and degrades with AI off. Scope is grounding (numbers + names), not full semantic judgement — labelled as such, no overclaim. Still open on this track: the full map-asset pipeline (9), grid-geometry automation (10), and the observability/cost-control layer (14).
- `Shipped (2026-06-22)`: **Phase 14 (Observability & Cost Controls)** — a repo-native telemetry/budget layer over the gateway client (`src/ai/aiObservability.ts`), no new dependency. A deterministic per-session budget tallies calls and estimated units (≈ chars/4, with image data URLs counted as a flat cost — no pinned provider prices/token tables) and short-circuits a call that would breach a configured ceiling to the typed `over-budget` failure BEFORE any network, so caps trip deterministically; ceilings are env-configurable (`VITE_AI_MAX_CALLS` / `VITE_AI_MAX_UNITS`) with finite, generous defaults. Every gateway call records an `AiTrace` linking a trace id to its task, prompt-template version (`PROMPT_TEMPLATE_VERSION`), estimated units, provider/model, latency, and result, kept in a bounded ring and surfaced read-only to the GM by `AiUsagePanel`. CI still exercises the whole path on recorded fixtures with no provider key. One honest nuance remains: stamping trace ids onto persisted scene events (the trace ring already correlates prompt → response → result → the user-visible meter by id) is a documented follow-on, not yet wired. Still open on this track: the full map-asset pipeline (9) and grid-geometry automation (10).
- `Shipped (2026-06-22)`: **Phase 9 (Generated Map Pipeline MVP)** — a scene can carry a background battle map. The image is content-addressed (`src/scene/mapAssets.ts`): its bytes live on the document (`assets`, stored once, travelling with export/import), while a `map.set` event references it only by hash, so the log stays small and a replay always resolves to the same asset. A `SceneMapRegistration` (asset hash + manual pixels-per-cell and origin offset) folds deterministically into `state.map`; the grid view (`SceneGridView`) overlays the art with percentage geometry so it scales with the responsive grid, and the GM imports/generates/aligns/clears it from a `MapPanel`. Import needs no provider key (a file → data URL); AI generation reuses the illustrate surface and is offered only when AI is on. The import boundary passes a map's data URL through the same scheme allowlist as character images (only `https:`/`data:image/*`), so a hostile import can't smuggle a beacon/script URL. Grid/geometry automation (10) builds on this registration model — shipped below.
- `Shipped (2026-06-22)`: **Phase 10 (Vision/Grid Automation)** — optional AI vision that PROPOSES a grid registration plus terrain/hazard/cover/spawn region boxes from a map image, gated by a deterministic geometry validator (`src/scene/mapAnalysis.ts`): the proposed grid (origin + cells x pixels-per-cell) must fit inside the image and every box must fall within the grid in cell coordinates, or it is rejected with coded issues for ONE bounded repair. A sixth control-plane task (`analyze-map` — vision input, structured output) drives it (`analyzeMapFlow`); an accepted proposal is applied through NORMAL scene events (`set-map` + `add-marker` via `mapAnalysisToIntents`), re-validated on the event-sourced path so an out-of-bounds box can never land. Reachable from the `MapPanel` "Analyze with AI" button (AI-gated, map required); manual registration (Phase 9) stays the fallback when AI is off or nothing validates. **This completes the Scene Runtime / AI-DM track (Phases 0–14): deterministic runtime, manual + AI encounter building, server-side AI gateway with five task surfaces plus map analysis, tactical executor, LLM strategist, narration critic, observability/cost caps, and the map pipeline are all live.**
