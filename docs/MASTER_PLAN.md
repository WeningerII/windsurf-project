...
- `Shipped (2026-07-09)`: **Phase 1 shipped via PR #30 with enumerated deferrals.** Landed: the `useAppNav` shell-nav union (including a not-yet-consumed `surface:'scene'` state and `selectScene` reducer arm), `NewCharacterDialog`, header segment nav, sheet-open/close writer rewiring, the Alt+N repoint, ungated Import, and lazy-mount-on-first-visit for `SceneManager`. Deferred out of the PR despite the plan's one-atomic-PR definition of done: `LibraryScenesView`; the five-writer scene-selection lift (`src/components/SceneManager.tsx` still owns `selectedSceneId` internally, and `useAppNav`'s `selectScene` action exists unconsumed); the 18rem left-rail relocation; Export/Delete re-home plus per-card roster controls; the `phase1-*.spec` Playwright acceptance gates; task 11 in full — both the `performance.mark`/`performance.measure` instrumentation (no baseline exists in `src/` for the Phase-7 interaction-budget gate) and an explicit `sceneManagerChunk` bundle guard (only the indirect eager-index-chunk budget in `scripts/check-bundle-size.mjs` exists); the task-12 SceneManager.test.tsx + capabilityScenarios.test.tsx rewrites (outstanding by coupling — they rewrite against task 3's unlanded controlled-prop contract, and the task-3/task-12 atomicity rule binds them to the selection lift); and the Alt+1/2/3 surface shortcuts.
- One standing plan deviation: the Phase-1 keepalive uses `<div hidden>` (display:none) in `src/App.tsx` — the mechanism final-plan Findings 7+14 explicitly forbid — and the mandated three-way `performance.measure` comparison harness was never built, so the deviation is unmeasured.
- `Active implementation track`: Phase 2 is blocked on closing three Phase-1 deferrals first — the five-writer scene-selection lift, `LibraryScenesView` plus the left-rail relocation it triggers, and a plan-compliant keepalive mechanism.
