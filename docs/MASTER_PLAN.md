...
- `Shipped (2026-07-21)`: the AI gateway is now **provider-agnostic** — a provider registry (`netlify/functions/providerRegistry.mts`) resolves the adapter from an `AI_PROVIDER` env selector (Gemini plus a mock adapter), and a pluggable `RateLimitStore` (`netlify/functions/rateLimitStore.mts`, in-memory default plus an inert `RATE_LIMIT_STORE_URL` durable stub) is the first, secret-free slice of the Phase 14 cost-control layer. **RFC 007 (`docs/rfc/007-ai-dm-runtime.md`, Draft)** specifies the AI-DM runtime endpoint — the Phase 12 strategist plus the narration/adjudication loop — as bounded proposals over the scene runtime that deterministic validators must accept; it remains unimplemented pending acceptance. Turnkey operations runbooks for the Phase 14 observability surface (Sentry alert rules, Supabase backup/restore) landed under `docs/runbooks/`.
...
| `docs/rfc/007-ai-dm-runtime.md` | Draft RFC | Autonomous AI-DM runtime as the loop form of RFC 002's proposal contract, layered on the shipped RFC 006 scene runtime and RFC 005 resource pools: the model proposes, deterministic validators decide (every AI-DM action becomes a typed scene-action intent through `resolveSceneAction` before any event appends), determinism and seeded replay preserved, peer-system by construction, default-off behind `VITE_AI_ENABLED`; unimplemented pending acceptance |
