...
> expose, so it serves all seven equally. **D&D 5e is the first system to register
> an orchestrator** (`src/creation/dnd5eCreation.ts`, one edition-agnostic flow for
> 2014 and 2024); the remaining systems become creatable by registering their own
> orchestrator — no UI fork. The 5e-named rows in the legacy table below are
...
| 4C. Remaining Systems' Orchestrators | Apply the same registry pattern to the other systems | each system's loaders/templates/validators | The same shared creator renders PF2e/PF1e/3.5e/M&M/Daggerheart once each registers a `creation` orchestrator (its own steps — e.g. ancestry/background/class — mapped onto the agnostic contract). 5e 2014 already falls out of the edition-agnostic 5e orchestrator (no separate fork) | Per-system creation fixtures; the shared creator renders each system without modification; no regression to the 5e flow |
