...
export interface DocDriftTruth {
  ...
  /** Files under `src/data/` — generated SRD tables, regenerated by the encoders. */
...
export const COUNT_RULES: ExpectedTextRule[] = [
  // CLAUDE.md is read cold by every agent session as project instructions, so
  // it states its numbers inline rather than citing docs/generated/ the way the
  // narrative docs do. These three rules are what makes that safe: the numbers
  // are inline for the reader but owned by the gate.
  {
    path: 'CLAUDE.md',
    description: 'CLAUDE.md registered system count',
    expectedText: (truth) => `shared by ${truth.registeredSystemCount} registered game systems`,
  },
  {
    path: 'CLAUDE.md',
    description: 'CLAUDE.md generated-data file count',
    expectedText: (truth) => `${truth.dataFileCount} files under \`src/data/\``,
  },
  {
    path: 'CLAUDE.md',
    description: 'CLAUDE.md verify chain length',
    expectedText: (truth) => `the full CI gate, ${truth.verifyGateCount} steps`,
  },
  {
    path: 'README.md',
