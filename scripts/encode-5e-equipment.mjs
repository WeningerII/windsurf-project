...
async function main() {
  ...
  const existing = await import('../src/data/dnd/5e-2014/equipment/index.ts');
  const generatedNames = new Set();
  for (const stem of ['srd-equipment', 'srd-magic-items']) {
    try {
      const mod = await import(`../src/data/dnd/5e-2014/equipment/${stem}.ts`);
  ...
  for (const raw of equipment) {
    ...
    const description = (raw.desc ?? []).join('\n\n');
  ...
  const emit = (stem, exportName, items) => {
    ...
    writeFileSync(
      resolve(`src/data/dnd/5e-2014/equipment/${stem}.ts`),
