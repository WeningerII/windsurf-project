# Daggerheart Data Model

> 23 nodes · cohesion 0.21

## Key Concepts

- **daggerheart.ts** (48 connections) — `src/types/daggerheart.ts`
- **getDaggerheartSheetState.ts** (26 connections) — `src/systems/daggerheart/getDaggerheartSheetState.ts`
- **useDaggerheartSheetResources.ts** (25 connections) — `src/systems/daggerheart/useDaggerheartSheetResources.ts`
- **DaggerheartDomainCard** (12 connections) — `src/types/daggerheart.ts`
- **GetDaggerheartSheetStateProps** (11 connections) — `src/systems/daggerheart/getDaggerheartSheetState.ts`
- **DaggerheartWeapon** (11 connections) — `src/types/daggerheart.ts`
- **DaggerheartAncestry** (9 connections) — `src/types/daggerheart.ts`
- **DaggerheartClass** (9 connections) — `src/types/daggerheart.ts`
- **DaggerheartCommunity** (8 connections) — `src/types/daggerheart.ts`
- **DaggerheartConsumable** (7 connections) — `src/types/daggerheart.ts`
- **UseDaggerheartTemplateHandlersProps** (6 connections) — `src/systems/daggerheart/useDaggerheartTemplateHandlers.ts`
- **DaggerheartArmor** (6 connections) — `src/types/daggerheart.ts`
- **DaggerheartLoot** (6 connections) — `src/types/daggerheart.ts`
- **DaggerheartDomain** (5 connections) — `src/types/daggerheart.ts`
- **DaggerheartInventoryDefinitionBase** (3 connections) — `src/types/daggerheart.ts`
- **DaggerheartTier** (2 connections) — `src/types/daggerheart.ts`
- **DaggerheartAdversaryRole** (1 connections) — `src/types/daggerheart.ts`
- **DaggerheartDamageType** (1 connections) — `src/types/daggerheart.ts`
- **DaggerheartInventoryCategory** (1 connections) — `src/types/daggerheart.ts`
- **DaggerheartPassiveCondition** (1 connections) — `src/types/daggerheart.ts`
- **DaggerheartRange** (1 connections) — `src/types/daggerheart.ts`
- **DaggerheartSubclass** (1 connections) — `src/types/daggerheart.ts`
- **DaggerheartWeaponCategory** (1 connections) — `src/types/daggerheart.ts`

## Relationships

- [Sheet Resource Loading Hooks](Sheet_Resource_Loading_Hooks.md) (21 shared connections)
- [System Definitions & Types](System_Definitions_%26_Types.md) (15 shared connections)
- [Equipment & Feature Browsers](Equipment_%26_Feature_Browsers.md) (10 shared connections)
- [Mam3e Derived Math](Mam3e_Derived_Math.md) (5 shared connections)
- [Encounter & Initiative Panels](Encounter_%26_Initiative_Panels.md) (5 shared connections)
- [Tabs UI Component](Tabs_UI_Component.md) (4 shared connections)
- [Daggerheart Sheet Automation](Daggerheart_Sheet_Automation.md) (4 shared connections)
- [Daggerheart Contribution Ledger](Daggerheart_Contribution_Ledger.md) (3 shared connections)
- [Dnd5e Resource Loading Hooks](Dnd5e_Resource_Loading_Hooks.md) (2 shared connections)
- [Daggerheart Combatant Builders](Daggerheart_Combatant_Builders.md) (2 shared connections)
- [Dnd5e Equipment & Features UI](Dnd5e_Equipment_%26_Features_UI.md) (1 shared connections)
- [Dnd35e/Pf1e Derived Math](Dnd35e-Pf1e_Derived_Math.md) (1 shared connections)

## Source Files

- `src/systems/daggerheart/getDaggerheartSheetState.ts`
- `src/systems/daggerheart/useDaggerheartSheetResources.ts`
- `src/systems/daggerheart/useDaggerheartTemplateHandlers.ts`
- `src/types/daggerheart.ts`

## Audit Trail

- EXTRACTED: 201 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*