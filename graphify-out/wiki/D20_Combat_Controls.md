# D20 Combat Controls

> 72 nodes · cohesion 0.07

## Key Concepts

- **abilityMod()** (53 connections) — `src/utils/math.ts`
- **types.ts** (43 connections) — `src/registry/types.ts`
- **Pf2eDataModel** (42 connections) — `src/systems/pf2e/data-model.ts`
- **math.ts** (41 connections) — `src/utils/math.ts`
- **engine.ts** (39 connections) — `src/systems/pf2e/engine.ts`
- **RollResult** (28 connections) — `src/registry/types.ts`
- **formatMod()** (24 connections) — `src/utils/math.ts`
- **Pf2eOverview.tsx** (20 connections) — `src/systems/pf2e/components/Pf2eOverview.tsx`
- **Pf2eSkillsTab.tsx** (20 connections) — `src/systems/pf2e/components/Pf2eSkillsTab.tsx`
- **Pf2eSavesTab.tsx** (18 connections) — `src/systems/pf2e/components/Pf2eSavesTab.tsx`
- **SystemEngine** (17 connections) — `src/registry/types.ts`
- **DiceRollButton.tsx** (14 connections) — `src/components/DiceRollButton.tsx`
- **Dnd5eSkillsTab.tsx** (14 connections) — `src/systems/dnd5e/shared/components/Dnd5eSkillsTab.tsx`
- **.prepareData()** (14 connections) — `src/systems/pf2e/engine.ts`
- **Dnd5eSavesTab.tsx** (12 connections) — `src/systems/dnd5e/shared/components/Dnd5eSavesTab.tsx`
- **Pf2eAbilitiesTab.tsx** (12 connections) — `src/systems/pf2e/components/Pf2eAbilitiesTab.tsx`
- **definition.ts** (12 connections) — `src/systems/pf2e/definition.ts`
- **DiceRollButton()** (11 connections) — `src/components/DiceRollButton.tsx`
- **Pf2eEngine** (10 connections) — `src/systems/pf2e/engine.ts`
- **classSpellcasting.ts** (10 connections) — `src/utils/classSpellcasting.ts`
- **D20AbilitiesTab.tsx** (8 connections) — `src/systems/d20-legacy/components/D20AbilitiesTab.tsx`
- **.rollCheck()** (8 connections) — `src/systems/pf2e/engine.ts`
- **pf2e-engine.test.ts** (8 connections) — `src/__tests__/engines/pf2e-engine.test.ts`
- **DamageHealControl.tsx** (6 connections) — `src/components/DamageHealControl.tsx`
- **D20AbilitiesTab()** (6 connections) — `src/systems/d20-legacy/components/D20AbilitiesTab.tsx`
- *... and 47 more nodes in this community*

## Relationships

- [Daggerheart Combatant Builders](Daggerheart_Combatant_Builders.md) (31 shared connections)
- [Pf2e Sheet Tabs](Pf2e_Sheet_Tabs.md) (29 shared connections)
- [D20 Legacy System Engines](D20_Legacy_System_Engines.md) (25 shared connections)
- [Dnd35e Saves & Data Model](Dnd35e_Saves_%26_Data_Model.md) (19 shared connections)
- [Dnd5e2024 Engine & Hit Dice](Dnd5e2024_Engine_%26_Hit_Dice.md) (19 shared connections)
- [Combat Toggles & Conditions](Combat_Toggles_%26_Conditions.md) (16 shared connections)
- [Mam3e Data Model & Engine](Mam3e_Data_Model_%26_Engine.md) (12 shared connections)
- [System Registry & Renderer](System_Registry_%26_Renderer.md) (11 shared connections)
- [D20 Class Spellcasting](D20_Class_Spellcasting.md) (9 shared connections)
- [Skills Tab & Combat Math](Skills_Tab_%26_Combat_Math.md) (8 shared connections)
- [Sheet Header & Stat Cards](Sheet_Header_%26_Stat_Cards.md) (8 shared connections)
- [Dnd35e/Pf1e Derived Math](Dnd35e-Pf1e_Derived_Math.md) (7 shared connections)

## Source Files

- `src/__tests__/components/DamageHealControl.test.tsx`
- `src/__tests__/components/DiceRollButton.test.tsx`
- `src/__tests__/engines/pf2e-engine.test.ts`
- `src/__tests__/utils/classSpellcasting.test.ts`
- `src/components/DamageHealControl.tsx`
- `src/components/DiceRollButton.tsx`
- `src/registry/types.ts`
- `src/systems/d20-legacy/components/D20AbilitiesTab.tsx`
- `src/systems/dnd5e/shared/components/Dnd5eSavesTab.tsx`
- `src/systems/dnd5e/shared/components/Dnd5eSkillsTab.tsx`
- `src/systems/pf2e/components/Pf2eAbilitiesTab.tsx`
- `src/systems/pf2e/components/Pf2eOverview.tsx`
- `src/systems/pf2e/components/Pf2eProficiencyBadge.tsx`
- `src/systems/pf2e/components/Pf2eSavesTab.tsx`
- `src/systems/pf2e/components/Pf2eSkillsTab.tsx`
- `src/systems/pf2e/constants.ts`
- `src/systems/pf2e/data-model.ts`
- `src/systems/pf2e/definition.ts`
- `src/systems/pf2e/engine.ts`
- `src/types/game-systems.ts`

## Audit Trail

- EXTRACTED: 616 (100%)
- INFERRED: 3 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*