# Mam3e Data Model & Engine

> 64 nodes · cohesion 0.08

## Key Concepts

- **sheet.tsx** (39 connections) — `src/systems/mam3e/sheet.tsx`
- **Mam3eDataModel** (33 connections) — `src/systems/mam3e/data-model.ts`
- **data-model.ts** (30 connections) — `src/systems/mam3e/data-model.ts`
- **useMam3eMutationHandlers.ts** (27 connections) — `src/systems/mam3e/useMam3eMutationHandlers.ts`
- **engine.ts** (22 connections) — `src/systems/mam3e/engine.ts`
- **useMam3eSheetController.ts** (18 connections) — `src/systems/mam3e/useMam3eSheetController.ts`
- **MamSkillsAdvantagesTab.tsx** (16 connections) — `src/systems/mam3e/components/MamSkillsAdvantagesTab.tsx`
- **mam3e-reference-surface-persistence.test.ts** (15 connections) — `src/__tests__/mam3e-reference-surface-persistence.test.ts`
- **MamComplicationsTab.tsx** (14 connections) — `src/systems/mam3e/components/MamComplicationsTab.tsx`
- **createDefaultMam3eData()** (14 connections) — `src/systems/mam3e/data-model.ts`
- **powers.ts** (14 connections) — `src/types/mam/powers.ts`
- **mam3eMutationHandlers.test.ts** (13 connections) — `src/__tests__/systems/mam3e/mam3eMutationHandlers.test.ts`
- **definition.ts** (12 connections) — `src/systems/mam3e/definition.ts`
- **getMam3eSheetState.ts** (12 connections) — `src/systems/mam3e/getMam3eSheetState.ts`
- **useMam3eMutationHandlers()** (11 connections) — `src/systems/mam3e/useMam3eMutationHandlers.ts`
- **Power** (11 connections) — `src/types/mam/powers.ts`
- **mam3eSheetShared.ts** (10 connections) — `src/systems/mam3e/mam3eSheetShared.ts`
- **mam3eSheetHelpers.test.ts** (10 connections) — `src/__tests__/systems/mam3e/mam3eSheetHelpers.test.ts`
- **MamAbilitiesTab.tsx** (9 connections) — `src/systems/mam3e/components/MamAbilitiesTab.tsx`
- **Mam3eEngine** (9 connections) — `src/systems/mam3e/engine.ts`
- **useMam3eSheetController()** (8 connections) — `src/systems/mam3e/useMam3eSheetController.ts`
- **mam3e-engine.test.ts** (8 connections) — `src/__tests__/engines/mam3e-engine.test.ts`
- **MamHeader.tsx** (7 connections) — `src/systems/mam3e/components/MamHeader.tsx`
- **Mam3eConditionTrack** (7 connections) — `src/systems/mam3e/data-model.ts`
- **getMam3eSheetState()** (7 connections) — `src/systems/mam3e/getMam3eSheetState.ts`
- *... and 39 more nodes in this community*

## Relationships

- [Daggerheart Combatant Builders](Daggerheart_Combatant_Builders.md) (40 shared connections)
- [Mam Powers & Cost Ledger](Mam_Powers_%26_Cost_Ledger.md) (38 shared connections)
- [SRD Manifest Generator](SRD_Manifest_Generator.md) (23 shared connections)
- [D20 Combat Controls](D20_Combat_Controls.md) (12 shared connections)
- [Combat & Recap Panels](Combat_%26_Recap_Panels.md) (5 shared connections)
- [System Registry & Renderer](System_Registry_%26_Renderer.md) (5 shared connections)
- [Sheet Header & Stat Cards](Sheet_Header_%26_Stat_Cards.md) (5 shared connections)
- [5e Feat Browser](5e_Feat_Browser.md) (4 shared connections)
- [Combat Toggles & Conditions](Combat_Toggles_%26_Conditions.md) (4 shared connections)
- [Sync Engine Tests](Sync_Engine_Tests.md) (4 shared connections)
- [App Shell & Layout](App_Shell_%26_Layout.md) (4 shared connections)
- [Browser Compat & Error Logging](Browser_Compat_%26_Error_Logging.md) (3 shared connections)

## Source Files

- `src/__tests__/engines/mam3e-engine.test.ts`
- `src/__tests__/mam3e-reference-surface-persistence.test.ts`
- `src/__tests__/systems/mam3e/mam3eMutationHandlers.test.ts`
- `src/__tests__/systems/mam3e/mam3eSheetHelpers.test.ts`
- `src/systems/mam3e/components/MamAbilitiesTab.tsx`
- `src/systems/mam3e/components/MamComplicationsTab.tsx`
- `src/systems/mam3e/components/MamConditionsTab.tsx`
- `src/systems/mam3e/components/MamHeader.tsx`
- `src/systems/mam3e/components/MamNotesTab.tsx`
- `src/systems/mam3e/components/MamSkillsAdvantagesTab.tsx`
- `src/systems/mam3e/data-model.ts`
- `src/systems/mam3e/definition.ts`
- `src/systems/mam3e/engine.ts`
- `src/systems/mam3e/getMam3eSheetState.ts`
- `src/systems/mam3e/mam3eSheetShared.ts`
- `src/systems/mam3e/sheet.tsx`
- `src/systems/mam3e/useMam3eMutationHandlers.ts`
- `src/systems/mam3e/useMam3eSheetController.ts`
- `src/types/mam/powers.ts`

## Audit Trail

- EXTRACTED: 471 (100%)
- INFERRED: 2 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*