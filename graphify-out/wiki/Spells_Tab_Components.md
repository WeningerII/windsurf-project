# Spells Tab Components

> 23 nodes · cohesion 0.15

## Key Concepts

- **Pf2eSpellsTab.tsx** (26 connections) — `src/systems/pf2e/components/Pf2eSpellsTab.tsx`
- **D20SpellsTab.tsx** (22 connections) — `src/systems/d20-legacy/components/D20SpellsTab.tsx`
- **spellPreparation.ts** (15 connections) — `src/utils/spellPreparation.ts`
- **buildSpellPreparationConcepts()** (9 connections) — `src/utils/spellPreparation.ts`
- **D20SpellsTab()** (8 connections) — `src/systems/d20-legacy/components/D20SpellsTab.tsx`
- **resolveSpellPreparationEntry()** (7 connections) — `src/utils/spellPreparation.ts`
- **Pf2eSpellcasting** (4 connections) — `src/systems/pf2e/data-model.ts`
- **Props** (3 connections) — `src/systems/pf2e/components/Pf2eSpellsTab.tsx`
- **PF2E_SPELLS_COPY** (3 connections) — `src/utils/documentationCopy.ts`
- **resolveSpellPreparationEntries()** (3 connections) — `src/utils/spellPreparation.ts`
- **compareSpells()** (2 connections) — `src/systems/d20-legacy/components/D20SpellsTab.tsx`
- **formatSpellLevel()** (2 connections) — `src/systems/d20-legacy/components/D20SpellsTab.tsx`
- **titleCase()** (2 connections) — `src/systems/d20-legacy/components/D20SpellsTab.tsx`
- **D20_ARCANE_SCHOOLS** (2 connections) — `src/utils/d20LegacySpellcasting.ts`
- **compareSpellEntries()** (2 connections) — `src/utils/spellPreparation.ts`
- **dedupeSpellIds()** (2 connections) — `src/utils/spellPreparation.ts`
- **humanizeSpellId()** (2 connections) — `src/utils/spellPreparation.ts`
- **SpellLookup** (2 connections) — `src/utils/spellPreparation.ts`
- **Pf2eSpellsTabComponent** (1 connections) — `src/systems/pf2e/components/Pf2eSpellsTab.tsx`
- **prunePreparedRanks()** (1 connections) — `src/systems/pf2e/components/Pf2eSpellsTab.tsx`
- **sortSpells()** (1 connections) — `src/systems/pf2e/components/Pf2eSpellsTab.tsx`
- **SpellPreparationConcepts** (1 connections) — `src/utils/spellPreparation.ts`
- **SpellPreparationEntry** (1 connections) — `src/utils/spellPreparation.ts`

## Relationships

- [Dnd35e Saves & Data Model](Dnd35e_Saves_%26_Data_Model.md) (6 shared connections)
- [Combat Toggles & Conditions](Combat_Toggles_%26_Conditions.md) (5 shared connections)
- [Spell Browser UI](Spell_Browser_UI.md) (5 shared connections)
- [Pf2e Spell Types & Traits](Pf2e_Spell_Types_%26_Traits.md) (4 shared connections)
- [Spell Catalog Consistency Tests](Spell_Catalog_Consistency_Tests.md) (4 shared connections)
- [Pf2e Sheet Tabs](Pf2e_Sheet_Tabs.md) (4 shared connections)
- [D20 Combat Controls](D20_Combat_Controls.md) (4 shared connections)
- [Currency & Inventory Editors](Currency_%26_Inventory_Editors.md) (3 shared connections)
- [D20 Legacy Spell Slots](D20_Legacy_Spell_Slots.md) (3 shared connections)
- [D20 Class Spellcasting](D20_Class_Spellcasting.md) (2 shared connections)
- [Sheet Resource Loading Hooks](Sheet_Resource_Loading_Hooks.md) (2 shared connections)
- [App Shell & Layout](App_Shell_%26_Layout.md) (2 shared connections)

## Source Files

- `src/systems/d20-legacy/components/D20SpellsTab.tsx`
- `src/systems/pf2e/components/Pf2eSpellsTab.tsx`
- `src/systems/pf2e/data-model.ts`
- `src/utils/d20LegacySpellcasting.ts`
- `src/utils/documentationCopy.ts`
- `src/utils/spellPreparation.ts`

## Audit Trail

- EXTRACTED: 117 (97%)
- INFERRED: 4 (3%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*