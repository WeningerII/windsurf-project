# Document Storage (IndexedDB)

> 11 nodes · cohesion 0.22

## Key Concepts

- **D20InventoryTab.tsx** (10 connections) — `src/systems/d20-legacy/components/D20InventoryTab.tsx`
- **InventoryManager.tsx** (9 connections) — `src/components/InventoryManager.tsx`
- **InventoryManager()** (5 connections) — `src/components/InventoryManager.tsx`
- **InventoryManager.test.tsx** (3 connections) — `src/__tests__/components/InventoryManager.test.tsx`
- **D20InventoryTab()** (2 connections) — `src/systems/d20-legacy/components/D20InventoryTab.tsx`
- **InventoryItem** (1 connections) — `src/components/InventoryManager.tsx`
- **InventoryManagerProps** (1 connections) — `src/components/InventoryManager.tsx`
- **Currency** (1 connections) — `src/systems/d20-legacy/components/D20InventoryTab.tsx`
- **InventoryItem** (1 connections) — `src/systems/d20-legacy/components/D20InventoryTab.tsx`
- **Props** (1 connections) — `src/systems/d20-legacy/components/D20InventoryTab.tsx`
- **existingItems** (1 connections) — `src/__tests__/components/InventoryManager.test.tsx`

## Relationships

- [Combat & Recap Panels](Combat_%26_Recap_Panels.md) (2 shared connections)
- [Combat Toggles & Conditions](Combat_Toggles_%26_Conditions.md) (2 shared connections)
- [Dnd35e/Pf1e Derived Math](Dnd35e-Pf1e_Derived_Math.md) (2 shared connections)
- [Currency & Inventory Editors](Currency_%26_Inventory_Editors.md) (2 shared connections)
- [Browser Compat & Error Logging](Browser_Compat_%26_Error_Logging.md) (1 shared connections)

## Source Files

- `src/__tests__/components/InventoryManager.test.tsx`
- `src/components/InventoryManager.tsx`
- `src/systems/d20-legacy/components/D20InventoryTab.tsx`

## Audit Trail

- EXTRACTED: 35 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*