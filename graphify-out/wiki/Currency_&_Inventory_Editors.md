# Currency & Inventory Editors

> 21 nodes · cohesion 0.14

## Key Concepts

- **D20LegacyTabs.tsx** (54 connections) — `src/systems/d20-legacy/components/D20LegacyTabs.tsx`
- **D20FeatsTab.tsx** (20 connections) — `src/systems/d20-legacy/components/D20FeatsTab.tsx`
- **Props** (10 connections) — `src/systems/d20-legacy/components/D20LegacyTabs.tsx`
- **Pf1eTrait** (10 connections) — `src/systems/pf1e/data-model.ts`
- **D20LegacyTabs()** (6 connections) — `src/systems/d20-legacy/components/D20LegacyTabs.tsx`
- **D20SavesTab()** (5 connections) — `src/systems/d20-legacy/components/D20SavesTab.tsx`
- **D20NotesTab.tsx** (4 connections) — `src/systems/d20-legacy/components/D20NotesTab.tsx`
- **D20SpellBrowserPanel** (4 connections) — `src/systems/d20-legacy/components/D20SpellBrowserPanel.tsx`
- **D20EquipmentBrowserTab** (3 connections) — `src/systems/d20-legacy/components/D20EquipmentBrowserTab.tsx`
- **D20FeatBrowserTab** (3 connections) — `src/systems/d20-legacy/components/D20FeatBrowserTab.tsx`
- **Props** (3 connections) — `src/systems/d20-legacy/components/D20FeatsTab.tsx`
- **D20_LEGACY_CONDITION_NAMES** (2 connections) — `src/rules/conditions/d20LegacyConditions.ts`
- **D20FeatsTab()** (2 connections) — `src/systems/d20-legacy/components/D20FeatsTab.tsx`
- **countTrainedSkills()** (2 connections) — `src/systems/d20-legacy/components/D20LegacyTabs.tsx`
- **D20NotesTab()** (2 connections) — `src/systems/d20-legacy/components/D20NotesTab.tsx`
- **FeatEntry** (1 connections) — `src/systems/d20-legacy/components/D20FeatsTab.tsx`
- **D20InventoryCurrency** (1 connections) — `src/systems/d20-legacy/components/D20LegacyTabs.tsx`
- **D20InventoryItem** (1 connections) — `src/systems/d20-legacy/components/D20LegacyTabs.tsx`
- **D20LegacyData** (1 connections) — `src/systems/d20-legacy/components/D20LegacyTabs.tsx`
- **Personality** (1 connections) — `src/systems/d20-legacy/components/D20NotesTab.tsx`
- **Props** (1 connections) — `src/systems/d20-legacy/components/D20NotesTab.tsx`

## Relationships

- [Dnd35e Saves & Data Model](Dnd35e_Saves_%26_Data_Model.md) (8 shared connections)
- [5e Feat Browser](5e_Feat_Browser.md) (7 shared connections)
- [Combat Toggles & Conditions](Combat_Toggles_%26_Conditions.md) (6 shared connections)
- [Dnd5e Feature List Sections](Dnd5e_Feature_List_Sections.md) (6 shared connections)
- [Equipped Armor Section](Equipped_Armor_Section.md) (5 shared connections)
- [Sheet Header & Stat Cards](Sheet_Header_%26_Stat_Cards.md) (5 shared connections)
- [Skills Tab & Combat Math](Skills_Tab_%26_Combat_Math.md) (4 shared connections)
- [Daggerheart Combatant Builders](Daggerheart_Combatant_Builders.md) (4 shared connections)
- [Dnd5e Equipment & Features UI](Dnd5e_Equipment_%26_Features_UI.md) (3 shared connections)
- [D20 Combat Controls](D20_Combat_Controls.md) (3 shared connections)
- [Spells Tab Components](Spells_Tab_Components.md) (3 shared connections)
- [D20 Class Spellcasting](D20_Class_Spellcasting.md) (3 shared connections)

## Source Files

- `src/rules/conditions/d20LegacyConditions.ts`
- `src/systems/d20-legacy/components/D20EquipmentBrowserTab.tsx`
- `src/systems/d20-legacy/components/D20FeatBrowserTab.tsx`
- `src/systems/d20-legacy/components/D20FeatsTab.tsx`
- `src/systems/d20-legacy/components/D20LegacyTabs.tsx`
- `src/systems/d20-legacy/components/D20NotesTab.tsx`
- `src/systems/d20-legacy/components/D20SavesTab.tsx`
- `src/systems/d20-legacy/components/D20SpellBrowserPanel.tsx`
- `src/systems/pf1e/data-model.ts`

## Audit Trail

- EXTRACTED: 136 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*