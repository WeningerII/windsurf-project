# Browser Compat & Error Logging

> 72 nodes · cohesion 0.05

## Key Concepts

- **browserCompat.ts** (23 connections) — `src/utils/browserCompat.ts`
- **ErrorLogger** (22 connections) — `src/utils/errorLogger.ts`
- **errorLogger.ts** (21 connections) — `src/utils/errorLogger.ts`
- **ErrorBoundary.tsx** (18 connections) — `src/components/ErrorBoundary.tsx`
- **generate-legal-notice.ts** (17 connections) — `src/scripts/generate-legal-notice.ts`
- **LegalNotices.tsx** (13 connections) — `src/components/LegalNotices.tsx`
- **Card.tsx** (12 connections) — `src/components/ui/Card.tsx`
- **ErrorCategory** (12 connections) — `src/utils/errorLogger.ts`
- **ErrorSeverity** (12 connections) — `src/utils/errorLogger.ts`
- **browserCompat.test.ts** (11 connections) — `src/__tests__/browserCompat.test.ts`
- **MonsterStatBlock.tsx** (10 connections) — `src/components/MonsterStatBlock.tsx`
- **ErrorBoundary** (8 connections) — `src/components/ErrorBoundary.tsx`
- **attributions.ts** (7 connections) — `src/legal/attributions.ts`
- **ErrorBoundary.test.tsx** (7 connections) — `src/__tests__/components/ErrorBoundary.test.tsx`
- **initBrowserCompat()** (7 connections) — `src/utils/browserCompat.ts`
- **MonsterStatBlock.test.tsx** (6 connections) — `src/__tests__/components/MonsterStatBlock.test.tsx`
- **errorLogger.test.ts** (6 connections) — `src/__tests__/errorLogger.test.ts`
- **errorLogger.test.ts** (6 connections) — `src/__tests__/utils/errorLogger.test.ts`
- **Card** (5 connections) — `src/components/ui/Card.tsx`
- **CardContent** (5 connections) — `src/components/ui/Card.tsx`
- **CardHeader** (5 connections) — `src/components/ui/Card.tsx`
- **CardTitle** (5 connections) — `src/components/ui/Card.tsx`
- **.log()** (5 connections) — `src/utils/errorLogger.ts`
- **guardSync()** (5 connections) — `src/utils/errorLogger.ts`
- **LegalAttributions** (4 connections) — `src/legal/attributions.ts`
- *... and 47 more nodes in this community*

## Relationships

- [Combat & Recap Panels](Combat_%26_Recap_Panels.md) (13 shared connections)
- [Game System Selector](Game_System_Selector.md) (9 shared connections)
- [Encounter & Initiative Panels](Encounter_%26_Initiative_Panels.md) (8 shared connections)
- [Dnd5e Resource Loading Hooks](Dnd5e_Resource_Loading_Hooks.md) (6 shared connections)
- [Monster & NPC Generator](Monster_%26_NPC_Generator.md) (5 shared connections)
- [App Shell & Layout](App_Shell_%26_Layout.md) (5 shared connections)
- [System Registry & Renderer](System_Registry_%26_Renderer.md) (4 shared connections)
- [Sheet Resource Loading Hooks](Sheet_Resource_Loading_Hooks.md) (4 shared connections)
- [Document Migrations & Storage](Document_Migrations_%26_Storage.md) (4 shared connections)
- [Mam3e Data Model & Engine](Mam3e_Data_Model_%26_Engine.md) (3 shared connections)
- [Sync Engine Tests](Sync_Engine_Tests.md) (1 shared connections)
- [Document Storage (IndexedDB)](Document_Storage_%28IndexedDB%29.md) (1 shared connections)

## Source Files

- `src/__tests__/LegalNotices.test.tsx`
- `src/__tests__/browserCompat.test.ts`
- `src/__tests__/components/ErrorBoundary.test.tsx`
- `src/__tests__/components/MonsterStatBlock.test.tsx`
- `src/__tests__/errorLogger.test.ts`
- `src/__tests__/utils/errorLogger.test.ts`
- `src/components/ErrorBoundary.tsx`
- `src/components/LegalNotices.tsx`
- `src/components/MonsterStatBlock.tsx`
- `src/components/ui/Card.tsx`
- `src/legal/attributions.ts`
- `src/scripts/generate-legal-notice.ts`
- `src/utils/browserCompat.ts`
- `src/utils/errorLogger.ts`

## Audit Trail

- EXTRACTED: 335 (100%)
- INFERRED: 1 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*