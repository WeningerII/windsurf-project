# EffectInstance

> God node · 61 connections · `src/rules/ir/types.ts`

**Community:** [Character Effects Compilation](Character_Effects_Compilation.md)

## Connections by Relation

### contains
- types.ts `EXTRACTED`

### imports
- sceneCombat.ts `EXTRACTED`
- characterCombatant.ts `EXTRACTED`
- systemProfiles.ts `EXTRACTED`
- monsterCombatant.ts `EXTRACTED`
- sceneCombatBridge.test.ts `EXTRACTED`
- sceneCombatBridgeUi.test.ts `EXTRACTED`
- participantResolution.ts `EXTRACTED`
- resolve.ts `EXTRACTED`
- tacticalExecutor.ts `EXTRACTED`
- attackResolution.ts `EXTRACTED`
- tacticalExecutor.test.ts `EXTRACTED`
- daggerheartCombatant.ts `EXTRACTED`
- modifierEffects.ts `EXTRACTED`
- roundDriver.ts `EXTRACTED`
- participantResolution.test.ts `EXTRACTED`
- d20LegacyConditions.ts `EXTRACTED`
- sceneConditions.ts `EXTRACTED`
- targetScoring.ts `EXTRACTED`
- mam3eCombatant.ts `EXTRACTED`
- sceneTerrain.ts `EXTRACTED`

### re_exports
- index.ts `EXTRACTED`

### references
- [GameSystemId](GameSystemId.md) `EXTRACTED`
- AttackResolution `EXTRACTED`
- TacticalActor `EXTRACTED`
- ContributionCategory `EXTRACTED`
- ContributionManualBoundary `EXTRACTED`
- RoundCombatant `EXTRACTED`
- SceneCombatStats `EXTRACTED`
- AttackResolutionInput `EXTRACTED`
- ResolveResult `EXTRACTED`
- CharacterCombatant `EXTRACTED`
- DaggerheartAdversaryCombatant `EXTRACTED`
- DaggerheartCombatant `EXTRACTED`
- Mam3eCombatant `EXTRACTED`
- MonsterCombatant `EXTRACTED`
- DaggerheartAttackInput `EXTRACTED`
- Mam3eAttackInput `EXTRACTED`
- AttackTarget `EXTRACTED`
- LegacyConditionTemplate `EXTRACTED`
- ConditionEffectTemplate `EXTRACTED`
- DaggerheartAttackResult `EXTRACTED`

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*