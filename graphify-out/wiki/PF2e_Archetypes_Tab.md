# PF2e Archetypes Tab

> 7 nodes · cohesion 0.43

## Key Concepts

- **usePwaInstallPrompt.ts** (8 connections) — `src/hooks/usePwaInstallPrompt.ts`
- **usePwaInstallPrompt()** (6 connections) — `src/hooks/usePwaInstallPrompt.ts`
- **isStandaloneMode()** (2 connections) — `src/hooks/usePwaInstallPrompt.ts`
- **readDismissedState()** (2 connections) — `src/hooks/usePwaInstallPrompt.ts`
- **writeDismissedState()** (2 connections) — `src/hooks/usePwaInstallPrompt.ts`
- **DeferredInstallPromptEvent** (1 connections) — `src/hooks/usePwaInstallPrompt.ts`
- **UsePwaInstallPromptOptions** (1 connections) — `src/hooks/usePwaInstallPrompt.ts`

## Relationships

- [Combat & Recap Panels](Combat_%26_Recap_Panels.md) (2 shared connections)
- [System Registry & Renderer](System_Registry_%26_Renderer.md) (1 shared connections)
- [App Shell & Layout](App_Shell_%26_Layout.md) (1 shared connections)

## Source Files

- `src/hooks/usePwaInstallPrompt.ts`

## Audit Trail

- EXTRACTED: 22 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*