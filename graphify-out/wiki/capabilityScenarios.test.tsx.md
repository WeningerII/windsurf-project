# capabilityScenarios.test.tsx

> 23 nodes · cohesion 0.12

## Key Concepts

- **capabilityScenarios.test.tsx** (72 connections) — `src/__tests__/scenarios/capabilityScenarios.test.tsx`
- **Campaign** (33 connections) — `src/types/core/campaign.ts`
- **SessionLog.tsx** (15 connections) — `src/components/campaign/SessionLog.tsx`
- **campaignStorySections.test.tsx** (9 connections) — `src/__tests__/components/campaignStorySections.test.tsx`
- **SessionLog()** (7 connections) — `src/components/campaign/SessionLog.tsx`
- **addSessionEntry()** (7 connections) — `src/utils/campaignStory.ts`
- **createSessionEntry()** (7 connections) — `src/utils/campaignStory.ts`
- **removeSessionEntry()** (4 connections) — `src/utils/campaignStory.ts`
- **Props** (2 connections) — `src/components/campaign/QuestList.tsx`
- **Props** (2 connections) — `src/components/campaign/SessionLog.tsx`
- **CampaignStorageData** (2 connections) — `src/utils/campaignStorage.ts`
- **ImportCampaignsResult** (2 connections) — `src/utils/campaignStorage.ts`
- **DATE_FORMAT** (1 connections) — `src/components/campaign/SessionLog.tsx`
- **baseCampaign()** (1 connections) — `src/__tests__/components/campaignStorySections.test.tsx`
- **QuestHost()** (1 connections) — `src/__tests__/components/campaignStorySections.test.tsx`
- **SessionHost()** (1 connections) — `src/__tests__/components/campaignStorySections.test.tsx`
- **BRUTE** (1 connections) — `src/__tests__/scenarios/capabilityScenarios.test.tsx`
- **counter()** (1 connections) — `src/__tests__/scenarios/capabilityScenarios.test.tsx`
- **makeCampaign()** (1 connections) — `src/__tests__/scenarios/capabilityScenarios.test.tsx`
- **makeDoc()** (1 connections) — `src/__tests__/scenarios/capabilityScenarios.test.tsx`
- **monster()** (1 connections) — `src/__tests__/scenarios/capabilityScenarios.test.tsx`
- **NOW** (1 connections) — `src/__tests__/scenarios/capabilityScenarios.test.tsx`
- **VICTIM** (1 connections) — `src/__tests__/scenarios/capabilityScenarios.test.tsx`

## Relationships

- [Quest & Session Log UI](Quest_%26_Session_Log_UI.md) (21 shared connections)
- [Combat & Recap Panels](Combat_%26_Recap_Panels.md) (11 shared connections)
- [Encounter & Initiative Panels](Encounter_%26_Initiative_Panels.md) (10 shared connections)
- [Scene Combat Resolution](Scene_Combat_Resolution.md) (7 shared connections)
- [System Registry & Renderer](System_Registry_%26_Renderer.md) (6 shared connections)
- [AI Encounter Drafting](AI_Encounter_Drafting.md) (6 shared connections)
- [Daggerheart Combatant Builders](Daggerheart_Combatant_Builders.md) (6 shared connections)
- [Campaign Storage & Hooks](Campaign_Storage_%26_Hooks.md) (6 shared connections)
- [AI Gateway Adapters](AI_Gateway_Adapters.md) (4 shared connections)
- [Dice Panel & Mam3e Resolution](Dice_Panel_%26_Mam3e_Resolution.md) (4 shared connections)
- [App Shell & Layout](App_Shell_%26_Layout.md) (4 shared connections)
- [Campaign Sync Hooks](Campaign_Sync_Hooks.md) (4 shared connections)

## Source Files

- `src/__tests__/components/campaignStorySections.test.tsx`
- `src/__tests__/scenarios/capabilityScenarios.test.tsx`
- `src/components/campaign/QuestList.tsx`
- `src/components/campaign/SessionLog.tsx`
- `src/types/core/campaign.ts`
- `src/utils/campaignStorage.ts`
- `src/utils/campaignStory.ts`

## Audit Trail

- EXTRACTED: 173 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*