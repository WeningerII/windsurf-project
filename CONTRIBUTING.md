...
- **Prose does not own status either.** The rule above was written for counts and only ever enforced for counts; status rotted the same way. A section marked `BLOCKED on §X` after §X was decided, a bullet asserting a module had no importers while another section described wiring it in, and a ledger item at `status: 'missing'` whose own cited evidence reported it complete all shipped in the same week. Two gates now catch that class — `blocked_ref_rule` and `ledger_status_rule` in `npm run check:doc-drift`. Status belongs in `docs/master-gap-ledger.source.ts`; plan prose cites it.
- **When you fix a class of defect, grep for its siblings before you close it.** This is the highest-yield habit in this list and the one with no gate behind it. Three defects in one week were each a lesson learned correctly in one place and applied nowhere else: the `.claude/worktrees` exclusion existed in `vitest.config.ts` (with a comment explaining exactly why) but not in `eslint.config.js` or `knip.jsonc`, which left `npm run verify` unable to complete locally; `check:keepalive-budget` counts operations rather than wall-clock for reasons `docs/MASTER_PLAN.md` records, while `gateBudget.test.tsx` shipped asserting on `performance.now()` and flaked 1 run in 3 under parallel workers. In every case the sibling was one `grep` away. A careful write-up of a fix feels like completion and becomes the reason nobody looks further — so search for the pattern, not the symptom.
- **And then check that the sibling fix actually worked.** The worktree example above is also a warning: copying the exclusion into `knip.jsonc` looked like the fix and was written up as one, but knip still OOMs with worktrees present — measured with that entry in place, 41 worktrees produce `FATAL ERROR: Reached heap limit` and removing them produces exit 0 in 7.8s (`docs/GAPS.md` §23.3). The habit found the right sibling; the remedy was wrong, and the confident comment left behind made it *less* likely anyone would re-examine it. **A fix that has never been observed failing before and passing after is a hypothesis.**
